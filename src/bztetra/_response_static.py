...
def _static_polarization_weights_on_local_mesh_pair_parallel_numba(
    ...
    pair_count = occupied_band_count * target_band_count
    local_weights = np.zeros(
        (local_point_count, target_band_count, occupied_band_count), dtype=np.float64
    )

    for pair_index in prange(pair_count):
        occupied_band_index = pair_index // target_band_count
        ...
        outer_weights = np.empty((1, 4), dtype=np.float64)
        ...
        secondary_weights = np.empty((1, 4), dtype=np.float64)
        ...
        for tetrahedron_index in range(tetrahedron_count):
            ...
            if sorted_occupied[0] <= 0.0 < sorted_occupied[1]:
                ...
                for vertex_index in range(4):
                    outer_weights[0, vertex_index] += secondary_weights[0, vertex_index]

            for point_index in range(20):
                total = 0.0
                for vertex_index in range(4):
                    total += (
                        outer_weights[0, vertex_index]
    ...
    for tetrahedron_index in range(tetrahedron_count):
        for occupied_band_index in range(occupied_band_count):
            ...
            if sorted_occupied[0] <= 0.0 < sorted_occupied[1]:
                ...
                for target_band_index in range(target_band_count):
                    for vertex_index in range(4):
                        outer_weights[target_band_index, vertex_index] += secondary_weights[
                            target_band_index, vertex_index
                        ]
            ...
            for target_band_index in range(target_band_count):
                for point_index in range(20):
                    ...
                    for vertex_index in range(4):
                        total += (
                            outer_weights[target_band_index, vertex_index]
                            * tetrahedron_weight_matrix[vertex_index, point_index]
                        )
                    point_weights[point_index] = total
                for point_index in range(20):
...
def _accumulate_small_tetra_dblstep_outer(
    ...
    for target_band_index in range(target_band_count):
        for column_index in range(4):
            ...
            for row_index in range(4):
                total += (
                    secondary_weights[target_band_index, row_index]
                    * outer_coefficients[row_index, column_index]
                )
...
def _double_step_secondary_weights_numba(
    ...
    for target_band_index in range(target_band_count):
        for vertex_index in range(4):
            difference[vertex_index] = (
                -occupied_vertices[vertex_index] + target_vertices[vertex_index, target_band_index]
            )
...
def _double_delta_weights_on_local_mesh_numba(
    ...
    target_band_count = target_tetra.shape[2]
    local_weights = np.zeros(
        (local_point_count, target_band_count, source_band_count), dtype=np.float64
    )
    ...
    if abs(sorted_differences[3] - sorted_differences[2]) < threshold:
        if abs(sorted_differences[3] - sorted_differences[1]) < threshold:
            if abs(sorted_differences[3] - sorted_differences[0]) < threshold:
                ...
                sorted_weights[3] = _polstat_1211_numba(
                ...
                sorted_weights[0] = _polstat_1222_numba(
            ...
            sorted_weights[3] = _polstat_1221_numba(
                sorted_differences[3], sorted_differences[1], logarithms[3], logarithms[1]
            )
            sorted_weights[2] = sorted_weights[3]
            sorted_weights[1] = _polstat_1221_numba(
        ...
        if abs(sorted_differences[2] - sorted_differences[0]) < threshold:
            sorted_weights[3] = _polstat_1222_numba(
                sorted_differences[3], sorted_differences[2], logarithms[3], logarithms[2]
            )
            sorted_weights[2] = _polstat_1211_numba(
...
@njit(cache=True)
def _polstat_1231_numba(
    g1: float, g2: float, g3: float, log1: float, log2: float, log3: float
) -> float:
